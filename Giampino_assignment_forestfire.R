...
n <- nrow(forest)
p <- ncol(x)
prop.mean.beta <- c(1,as.numeric(colMeans(forest)[-26]))
prop.sd.beta <- c(0.1,as.numeric(apply(forest, 2, function(col) sd(col))[-26]))
...
jags.hyper.u # intercept, DIC = 1829
...
jags.hyper.bb # intercept, DIC = 1828.092 
...
# "intercept" "FFMC"      "DC"        "temp"      "RH"        "wind" 
